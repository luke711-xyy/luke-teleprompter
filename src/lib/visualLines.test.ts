...
describe("focusedTwoLineTokenIds", () => {
  ...
  it("promotes the next line when the active token reaches the later part of the current line", () => {
    const measurements = [
      { id: 1, top: 100 },
      { id: 2, top: 100 },
      { id: 3, top: 100 },
    ...
    expect(focusedTwoLineTokenIds(measurements, 3, 72)).toEqual([4, 5, 6]);
    expect(leadingTwoLineTokenId(measurements, 3, 72)).toBe(4);
  });
});
